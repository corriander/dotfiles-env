...
- **MemPalace** — *a local HTTP MCP service on every machine*: one supervised server per machine owns that machine's palace and serves `http://127.0.0.1:8765/mcp` (`GET /healthz` liveness); clients never spawn their own server process. Configure clients with native HTTP. One exception, on any machine: Claude Desktop's local config only accepts stdio servers (its cloud-brokered connectors can't reach loopback), so it goes through a thin `mcp-remote` stdio→HTTP adapter until the app supports local HTTP config. The personal palace (WSL/Windows host) is authoritative; other machines run independent palaces, and cross-palace transfer is the ackb airlock's concern, not ad-hoc copying. Per-host deployment detail: ackb `docs/ops/mempalace-http.md`. `/handoff` and `/recall` behave identically everywhere.
