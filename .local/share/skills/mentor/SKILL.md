...
2. **Classify every delta** into exactly one of — **Preference is the default**:
   - **Correction** — only if you can cite the specific pre-existing constraint the model output violated (ADR spec, CONTEXT.md, prior decision) **or** point at an objective defect (factual error, typo, internal inconsistency). No citation → it is not a correction. If your citation attempt fails because the spec is silent, inconsistent, or the corpus has drifted from it, the finding routes to the **spec**, not the model — specs are fallible, and **corpus consistency is evidence, not authority** (verified 2026-07-06: a sibling-matching header "correction" was overturned because the README never codified the sibling style).
   - **Preference/framing** — legitimate output Alex wanted said differently. Guidance-shaped: belongs in whatever spec governs the artefact type, if it generalises at all.
   ...
   Calibration, both directions: a human editorial pass over subjective artefacts (ADRs, docs, prose) is **normal and expected, not evidence of model failure** — Alex refining output means he read it and performed the editorial pass. Don't self-flagellate in the note; equally, don't let expected-editing lower the authoring bar.
...
## Distillation (later phase — how the notes get used)

Every note already carries two orthogonal signals with no extra filing: **authoring tier** (the signature) and **task outcome** (the score). At distillation time, read them as a 2×2, not a single weight:

- high tier × high score → **gold**: distill into skills for the bank.
- low tier × high score → **strength to propagate down-tier** — the bidirectional case; don't lose it under tier-weighting.
- any tier × low score → **diagnosed anti-pattern / workflow gap**: mine for guidance and process fixes; never distill as method.

Skills are distilled from high-score notes only; sub-5 notes exist to drive continuous workflow improvement (missing context, inaccurate instructions, spec gaps). Respect each rule's `[judgement: …]` floor when deciding how far down-tier it ships.
